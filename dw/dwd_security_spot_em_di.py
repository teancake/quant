...
if __name__ == '__main__':
    ...
    ddl_sql_str = f'''        
    CREATE TABLE IF NOT EXISTS `dwd_security_spot_em_di` ( 
     `gmt_create` datetime NULL COMMENT "",
     `gmt_modified` datetime NULL COMMENT "",
     `代码` varchar(50) NULL COMMENT "",
     `名称` varchar(50) NULL COMMENT "",
     `最新价` double NULL COMMENT "",
     `涨跌额` double NULL COMMENT "",
     `涨跌幅` double NULL COMMENT "",
     `成交量` double NULL COMMENT "",
     `成交额` double NULL COMMENT "",
     `开盘价` double NULL COMMENT "",
     `最高价` double NULL COMMENT "",
     `最低价` double NULL COMMENT "",
     `昨收` double NULL COMMENT "",
    ...
    '''
    dwd_sql_str = f'''
    INSERT OVERWRITE dwd_security_spot_em_di PARTITION(p{ds})
    SELECT 
        gmt_create, 
        gmt_modified, 
        代码,
        ...
        'stock_zh_a' as type,
        ds
    FROM dwd_stock_zh_a_spot_em_di 
    WHERE ds = {ds}
    UNION ALL 
    SELECT 
        gmt_create, 
    ...
    db.dqc_row_count("dwd_security_spot_em_di", ds)
