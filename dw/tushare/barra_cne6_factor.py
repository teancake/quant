...
from dw.tushare.data_fetcher import LocalData, DbData
...
class lazyproperty:
    ...
    def __get__(self, instance, cls):
        if instance is None:
            ...
            logger.info(f"call function {self.func.__name__}")
...
class parallelcal:
    ...
    def nanfunc(self, series, func, sentinel=SENTINEL, weights=None):
        valid_idx = np.argwhere(series != sentinel).flatten()
    ...
    def _cal_liquidity(series, days_pm=21, sentinel=-SENTINEL):
        ...
        series = series[valid_idx.flatten()]
...
class CALFUNC(LocalData):
    ...
    @lazyproperty
    def tdays(self):
        return sorted(self.adjfactor.index)
    ...
    def _transfer_freq(self, datdf, method='lyr', from_='q', to_='d', start_year=2006):
        ...
        if from_ == 'q':
            if method == 'mrq':
                ...
                res = pd.DataFrame()
                for gp in ['Q1', 'Q2', 'Q3']:
                    tmp = dat_cleaned.groupby(gp, group_keys=False).apply(self.fill_vals).dropna(how='all', axis=0)
                    tmp = tmp.groupby(gp, group_keys=False).apply(self.clear_vals).apply(self.fill_vals)
                    tmp = tmp.iloc[:, :-3]
                ...
                res = pd.DataFrame()
                for gp, df in dat_grouped.groupby('Q4-1', group_keys=False):
    ...
    def get_trade_days(self, start_year, end_year, tdays=None):
        if tdays is None:
            tdays = self.tdays
        start_year_dt = pd.to_datetime(f'{start_year}-01-01')
        end_year_dt = pd.to_datetime(f'{end_year}-12-31')
        return [day for day in tdays if start_year_dt <= day <= end_year_dt]
    ...
    def __drop_invalid_and_fill_val(series, val=None, method=None):
        ...
        series = pd.concat([series.iloc[:valid_idx[0]], series_valid])
        return series
...
class Momentum(CALFUNC):
    @lazyproperty
    def STREV(self):
        strev = self._rolling(self.changepct / 100, window=21, half_life=5, func_name='sum')
        return strev
    ...
    def INDMOM(self):
        ...
        dat.index.set_names(["time", "code"], inplace=True)
...
class EarningsVariablity(Quality):
    ...
    def VSAL(self):
        ...
        vsal = self._transfer_freq(vsal, method='mrq', from_='q', to_='d')
        return vsal.T
    ...
    def VERN(self):
        ...
        vern = self._transfer_freq(vern, method='mrq', from_='q', to_='d')
    ...
    def VFLO(self):
        ...
        vflo = self._transfer_freq(vflo, method='mrq', from_='q', to_='d')
...
class Profitability(Quality):
    ...
    @lazyproperty
    def GP(self):
        sales = self._transfer_freq(self.operatingreenue, method='mrq', from_='q', to_='d')
        cogs = self._transfer_freq(self.cogs_q, method='mrq', from_='q', to_='d')
        ta = self._transfer_freq(self.totalassets, method='mrq', from_='q', to_='d')

        # sales = self._transfer_freq(self.operatingreenue, method='lyr', from_='q', to_='d')
        # cogs = self._transfer_freq(self.cogs_q, method='lyr', from_='q', to_='d')
        # ta = self._transfer_freq(self.totalassets, method='lyr', from_='q', to_='d')
...
class InvestmentQuality(Quality):
    ...
    @lazyproperty
    def AGRO(self):
        agro = self._get_growth_rate(self.totalassets, periods=self.window, freq='y')
        return agro
...
class EarningsYield(Value):
    ...
    def EM(self):
        ...
        ebit = self._transfer_freq(self.ebit, method='mrq', from_='q', to_='d')
...
if __name__ == '__main__':
    ...
    for style in factor_styles:
        ...
        for factor in factors_names:
            print(f"fstyle {fstyle}, factor {factor}")
            # if VERSION == 5:
